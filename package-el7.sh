...
if [[ "$(docker images -q cockpit-samba-manager-el7-builder 2> /dev/null)" == "" ]]; then
	docker build -t cockpit-samba-manager-el7-builder - < docker/el7
	res=$?
	if [ $res -ne 0 ]; then
...
docker run -u $(id -u):$(id -g) -w /home/rpm/rpmbuild -it -v$(pwd)/dist/tmp:/home/rpm/rpmbuild/SOURCES -v$(pwd)/dist/el7:/home/rpm/rpmbuild/RPMS -v$(pwd)/el:/home/rpm/rpmbuild/SPECS --rm cockpit-samba-manager-el7-builder rpmbuild -ba SPECS/cockpit-samba-manager.spec
res=$?
if [ $res -ne 0 ]; then
